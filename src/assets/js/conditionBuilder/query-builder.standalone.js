    ...
    QueryBuilder.prototype.updateRuleOperator = function (rule, previousOperator) {
        ...
        if (!rule.operator || rule.operator.nb_inputs === 0) {
            ...
            if ($valueContainer.is(':empty') || !previousOperator ||
                ...
                (rule.filter.input === 'select' && rule.filter.multiple)
            ) {
                this.createRuleInput(rule);
            }
        }
...
<div id="{{= it.group_id }}" class="rules-group-container"> \
  <div class="rules-group-header"> \
    <div class="float-right group-actions"> \
      <a href="javascript:void(0)" title="{{= it.translate("add_rule") }}" data-add="rule"> \
        {{= it.icons.add_rule }} \
      </a> \
      {{? it.settings.allow_groups===-1 || it.settings.allow_groups>=it.level }} \
        <a href="javascript:void(0)" title="{{= it.translate("add_group") }}" data-add="group"> \
          {{= it.icons.add_group }} \
        </a> \
      {{?}} \
      {{? it.level>1 }} \
        <a href="javascript:void(0)" title="{{= it.translate("delete_group") }}" data-delete="group"> \
          {{= it.icons.remove_group }} \
        </a> \
      {{?}} \
    </div> \
    ...
    {{? it.settings.display_errors }} \
      <div class="error-container">{{= it.icons.error }}</div> \
    {{?}} \
  </div> \
...
<div id="{{= it.rule_id }}" class="rule-container"> \
  ...
  <div class="rule-header"> \
    <div class="btn-group float-right rule-actions"> \
      <a href="javascript:void(0)" data-delete="rule"> \
        {{= it.icons.remove_rule }} \
      </a> \
    </div> \
  </div> \
</div>';
...
{{ var optgroup = null; }} \
<select class="form-control form-control-sm" name="{{= it.rule.id }}_filter"> \
...
{{?}} \
{{ var optgroup = null; }} \
<select class="form-control form-control-sm {{? it.operators.length === 1 }}hide{{?}}" name="{{= it.rule.id }}_operator"> \
...
{{ var optgroup = null; }} \
<select class="form-control form-control-sm" name="{{= it.name }}" {{? it.rule.filter.multiple }} {{? it.operator.multiple}}multiple{{?}}{{?}}> \
  ...
  {{? optgroup !== null }}</optgroup>{{?}} \
    ...
    QueryBuilder.prototype.getRuleValueSelect = function (name, rule) {
        var h = this.templates.ruleValueSelect({
            ...
            name: name,
            rule: rule,
            operator: this.operators.find(x => x.type === rule.operator.type),
            icons: this.icons,
            settings: this.settings,
            translate: this.translate.bind(this)
        });
  ...
  <label for="' + id + '">' + val + '</label> \
    ...
    QueryBuilder.define('bt-selectpicker', function (options) {
        ...
        this.on('afterCreateRuleFilters', function (e, rule) {
            rule.$el.find(Selectors.rule_filter).removeClass('form-control form-control-sm').selectpicker(options);
        });

        this.on('afterCreateRuleOperators', function (e, rule) {
            rule.$el.find(Selectors.rule_operator).removeClass('form-control form-control-sm').selectpicker(options);
        });
    ...
    QueryBuilder.define('chosen-selectpicker', function (options) {
        ...
        // init selectpicker
        this.on('afterCreateRuleFilters', function (e, rule) {
            rule.$el.find(Selectors.rule_filter).removeClass('form-control form-control-sm').chosen(options);
        });

        this.on('afterCreateRuleOperators', function (e, rule) {
            if (e.builder.getOperators(rule.filter).length > 1) {
                rule.$el.find(Selectors.rule_operator).removeClass('form-control form-control-sm').chosen(options);
            }
        });
